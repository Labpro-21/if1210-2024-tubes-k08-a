            ...
            if aksi_beli_monster == type_inv_id:
                ...
                if stok >= 1:
                    if oc >= price:
                        ...
                        print(f"Sisa O.W.C.A kamu {oc-price}")
                        oc = oc - price
...
def beli_potion():
    ...
    if aksi_beli_potion == id_1:
        oc = int(oc)
        harga_1 = int(harga_1)
        jumlah = jumlah_beli_potion * harga_1
        if oc >= harga_1:
            print(f"kamu berhasil membeli {type_1}")
            print(f"Sisa O.W.C.A kamu {oc-jumlah}")
            oc = oc - jumlah
            baca_oc[pilih_id][4] = oc
        elif oc < harga_1:
            print("O.W.C.A kamu kurang :(")
...
mons_inv = read_csv("monster_inventor.csv")
...
while x == 0:
    ...
    if aksi == "keluar":
        ...
        if aksi_3 == "monster" or aksi_3 == "Monster" or aksi_3 == "MONSTER":
            ...
            beli_mons()
        if aksi_3 == "potion" or aksi_3 == "Potion" or aksi_3 == "POTION":
            aksi_beli_potion = str(input("Masukkan id potion: "))
